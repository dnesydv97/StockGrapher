...
const Home = () => {
  ...
  console.log('selected', selectedChartItem);
  ...
  useEffect(() => {
    if (details) {
      ...
      for (const date in details['Weekly Adjusted Time Series']) {
        const value =
          details['Weekly Adjusted Time Series'][date][selectedChartItem];

        valuesWithDates.push({date, value});
      }
      console.log('values with data', valuesWithDates);
      setData({
        labels:valuesWithDates.map((item) => item.date),
        datasets: [
          {
            data: valuesWithDates.map((item) => item.value),
          },
        ],
      });
